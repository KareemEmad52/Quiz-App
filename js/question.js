import {finalData ,quiz ,questionCont} from './index.js';
export class Question{
    constructor(index){
        this.question = finalData[index].question;
        this.answer = finalData[index].correct_answer;
        this.wrongAnwser = finalData[index].incorrect_answers;
        this.index = index;
        this.category = finalData[index].category;
        this.allAnswer = this.getAllAnswer();
        this.answered = false;
    }

    getAllAnswer(){
        return this.wrongAnwser.concat(this.answer).sort()
    }

    displayQuestion(){
        const questionMarkUp = `
    <div
      class="question shadow-lg col-lg-6 offset-lg-3  p-4 rounded-3 d-flex flex-column justify-content-center align-items-center gap-3 animate__animated animate__bounceIn"
    >
      <div class="w-100 d-flex justify-content-between">
        <span class="btn btn-category">${this.category}</span>
        <span class="fs-6 btn btn-questions">${this.index + 1} of ${finalData.length
      } Questions</span>
      </div>
      <h2 class="text-capitalize h4 text-center">${this.question}</h2>  
      <ul class="choices w-100 list-unstyled m-0 d-flex flex-wrap text-center">
      ${this.allAnswer.map((choice) => `<li>${choice}</li>`).join("")}
      </ul>
      <h2 class="text-capitalize text-center score-color h3 fw-bold"><i class="bi bi-emoji-laughing"></i> Score: ${quiz.score
      } </h2>        
    </div>
  `;

        questionCont.innerHTML = questionMarkUp
        let answerList = document.querySelectorAll('.question ul li')

        for(let i =0 ; i<answerList.length;i++){
            answerList[i].addEventListener('click',(e)=>{
                this.checkAnswer(e)
            })
        }
}




    checkAnswer(e){
        if (!this.answered){
            this.answered = true;
            if(e.target.innerHTML.toLowerCase() == this.answer.toLowerCase()){
                e.target.classList.add('correct','animate__animated','animate__flipInX')
                quiz.score += 1;
            } else {
                e.target.classList.add('wrong','animate__animated','animate__shakeX')
            }
    
            this.animateQuestion(e.target,1000)
        }
    }


    animateQuestion(element, duration){
        setTimeout(()=>{
            element.closest('.question').classList.replace('animate__bounceIn','animate__backOutLeft');
            setTimeout(()=>{
                this.nextQuestion();
            },500)
        },duration)
    }

    nextQuestion(){
        this.index = this.index +1;
        if(this.index > finalData.length -1){
            questionCont.innerHTML =  quiz.endQuiz();
            const tryAgainBtn = document.querySelector('.again');
            console.log(tryAgainBtn);
            tryAgainBtn.addEventListener('click' , ()=>{
                location.reload();
            })
            return
        }

    const x = new Question(this.index)
    x.displayQuestion()
    }
}